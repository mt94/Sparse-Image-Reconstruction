    ...
    def Generate(self, y):
        ...
        if self.snrDb is not None:
            ...
            if self.gaussianNoiseSigma is None:
                ...
                self.gaussianNoiseSigma = np.sqrt(float(yFlatSquaredSum)/float(y.size)/snr)
            else:
                # Set the snrDb field. This already may be correctly set, but that doesn't matter
                self.snrDb = 10*np.log10(float(yFlatSquaredSum)/float(y.size)/np.square(self.gaussianNoiseSigma))
