...
class AbstractSparseImageGenerator(AbstractImageGenerator):    
    ...
    def Generate(self):
        ...
        img = np.zeros(self.imageShape); # Init
        ...
        if (len(self.imageShape) == 2):
            ...
            fnPickLoc = lambda : (np.random.randint(validInterval[0, 0], validInterval[0, 1]),
            ...
            raise NotImplementedError('Cannot handle |imageShape|={0}'.format(len(self.imageShape)))
...
class SparseUniformImageGenerator(AbstractSparseImageGenerator):    
    ...
    # Abstract method override
    def SampleFromDistribution(self):
        return (self._uniformRvRange[0] + np.random.rand() * (self._uniformRvRange[1] - self._uniformRvRange[0]))        
