...
class T_PsfLinearDerivative(unittest.TestCase):
    ...
    def testMultiply(self):
        ...
        imgBinaryRandom = np.random.randint(2, size=T_PsfLinearDerivative.TEST_IMAGE_SHAPE)
        yBinaryRandomWoCorrection = gb.BlurImage(imgBinaryRandom)        
        self.assertNotAlmostEqual(0, np.mean(yBinaryRandomWoCorrection.flat), EPS_PRECISION_PLACES)
