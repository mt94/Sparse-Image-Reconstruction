...
class McmcIterationEvaluator(AbstractIterationsObserver):
    ...
    def UpdateState(self, stateDict):
        ...
        if all(k in stateDict for k in keysForMsg):      
            ...
            if self.bVerbose:
                print("=> Iter {0}: hyper samp.: w={1:.5f}, a={2:.5f}; var samp.: {3:.5e}".format(self.countIteration,
                                                                                                  wIter,
                                                                                                  aIter,
                                                                                                  noiseVarIter))
        if self.bVerbose \
        ...
        if (McmcIterationEvaluator.STATE_KEY_X_ITER in stateDict) and isinstance(stateDict[McmcIterationEvaluator.STATE_KEY_X_ITER], np.ndarray):
            ...
            if self.bVerbose:
                xSampledMsg += " n0={0} |x|_0={1} |x|_1={2:.5f} |xErr|_1={3:.5f}".format(n0Next,
                                                                                         n1Next,
                                                                                         np.sum(np.abs(xIter)),
                                                                                         xErrL1Norm
                                                                                         )                            
            if bPlot:
                # Plot xTrue
                plt.figure(self.xFigureNum); plt.imshow(np.reshape(self.xTrue, self.xShape), interpolation='none')
                plt.title('Actual theta')
                plt.colorbar()                                
                # Plot xIter
                plt.figure(); plt.imshow(np.reshape(xIter, self.xShape), interpolation='none')
                plt.title("xIter at iteration {0}".format(self.countIteration))
                if np.max(xIter.flat) > np.min(xIter.flat):
                    plt.colorbar()  
                # Plot xErrL1NormHistory
                plt.figure(); plt.semilogy(self.xErrL1NormHistory, 'b-'); plt.grid()
                plt.title('|xErr|_1 vs. iteration')              
                bPlotToShow = True
        
        if (McmcIterationEvaluator.STATE_KEY_HX_ITER in stateDict) and isinstance(stateDict[McmcIterationEvaluator.STATE_KEY_HX_ITER], np.ndarray):
            ...
            if bPlot:
                # Plot y
                plt.figure(self.xFigureNum + 3); plt.imshow(self.y, interpolation='none')
                plt.title('y')
                plt.colorbar()                
                # Plot H*xIter
                plt.figure(); plt.imshow(np.reshape(hxIter, self.xShape), interpolation='none')                
                ...
                plt.figure(); plt.imshow(yErr, interpolation='none')                
                plt.title('(y - H*xIter) at iteration{0}'.format(self.countIteration))
                if np.max(yErrFlat) > np.min(yErrFlat):
                    plt.colorbar()
                plt.figure(); plt.hist(yErrFlat, 20)
                plt.title('Histogram of (y - H*xIter) values at iteration{0}'.format(self.countIteration))
                # Plot yErrL2NormHistory
                plt.figure(); plt.semilogy(self.yErrL2NormHistory, 'r-'); plt.grid()
