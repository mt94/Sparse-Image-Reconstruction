...
class LarsLassoReconstructor(LarsReconstructor):
    ...
    @staticmethod
    def _CalculateSignViolationIndex(
        activeSet, betaHatActiveSet, wActiveSet, corrHatSign
    ):
        dHat = np.zeros(betaHatActiveSet.shape)
        dHat[activeSet] = np.transpose(
            corrHatSign[activeSet] * np.transpose(wActiveSet)
        )
        ...
        if indStrictlyPositive.size > 0:
            ...
            jTilde = None
            gammaTilde = np.Inf
        return {"indViolation": jTilde, "gammaViolation": gammaTilde, "d": dHat}
    ...
    def Iterate(self, y, fnConvolveWithPsf, fnConvolveWithPsfPrime):
        ...
        while numIter < maxIter:
            ...
            if lastIndexToJoinActiveSet is not None:
                assert (
                    lastIndexToJoinActiveSet.size == 1
                )  # Consequence of insisting that bEnforceOneatatimeJoin be True
                ...
                raise RuntimeError(
                    "Iteration {0}: no change to the active set scheduled".format(
                        numIter
                    )
                )
            ...
            a = fnConvolveWithPsfPrime(
                np.reshape(np.array(activeSetResult["u"]), y.shape)
            )
            aFlat = a.flat
            ...
            violationResult = LarsLassoReconstructor._CalculateSignViolationIndex(
                activeSet, betaHatActiveSet, activeSetResult["w"], corrHatSign
            )
            ...
            if violationResult["gammaViolation"] >= joinResult["gammaHat"]:
                ...
                if nVerbose >= 1:
                    msgBuffer.append(
                        "Sign violation: gammaHat is {0} vs. gammaTilde is {1}".format(
                            joinResult["gammaHat"], violationResult["gammaViolation"]
                        )
                    )
                if violationResult["indViolation"].size > 1:
                    raise RuntimeError(
                        "Lasso sign violation occurs at multiple indices in the active set"
                    )
                else:
                    lastIndexToJoinActiveSet = None
                    lastIndexToLeaveActiveSet = violationResult["indViolation"]
                    activeSetIndexToDelete = np.where(
                        activeSet == lastIndexToLeaveActiveSet
                    )[0]
                    assert activeSetIndexToDelete.size == 1
            ...
            if nVerbose >= 1:
                msgBuffer.append(
                    "gammaHat is {0}, AActiveSet is {1}, predicted corrHatAbsMax is is {2}".format(
                        gammaHatCurrentIter, activeSetResult["A"], corrHatAbsMax
                    )
                )
                ...
                iterObserver.UpdateState(
                    {
                        ...
                        LarsIterationEvaluator.FUNC_KEY_MU_FROM_THETA: lambda aTheta: fnConvolveWithPsf(
                            np.reshape(aTheta, y.shape)
                        ),
                    },
                    bPlotThetaErr=False,
                )
