...
class LarsReconstructor(AbstractReconstructor):
    ...
    def _CalculateActiveSetVariables(XActiveSet):
        ...
        assert np.allclose(
            np.array(XActiveSet.T * uActiveSet),
            AActiveSet * np.ones((activeSetSize, 1)),
        )
        return {"A": AActiveSet, "u": uActiveSet, "w": wActiveSet}

    @staticmethod
    def _CalculateNewJoinIndex(
        corrHatAbsMax, cj, AActiveSet, aj, activeSetComplement, EPS
    ):
        ...
        compMinusInComplement = (
            corrHatAbsMax - cj[activeSetComplement[indMinusDefined]]
        ) / (AActiveSet - aj[activeSetComplement[indMinusDefined]])
        compPlusInComplement = (
            corrHatAbsMax + cj[activeSetComplement[indPlusDefined]]
        ...
        if indMinusInComplementAndPos.size > 0:
            compMinusInComplementAndPos = compMinusInComplement[
                indMinusInComplementAndPos
            ]
        ...
        if (indMinusCandidate == -1) or (
            (gammaMinusCandidate is not None)
            and (gammaPlusCandidate is not None)
            and (gammaPlusCandidate < gammaMinusCandidate)
        ):
            indCandidate = activeSetComplement[
                indPlusDefined[indPlusInComplementAndPos[indPlusCandidate]]
            ]
            ...
            indCandidate = activeSetComplement[
                indMinusDefined[indMinusInComplementAndPos[indMinusCandidate]]
            ]
        ...
        indMinusClose = np.where(
            np.abs(compMinusInComplement - gammaCandidate) < EPS * 2
        )[0]
        if indMinusClose.size > 0:
            ...
            if indTmp.size > 0:
                ...
                gammaCandidateOtherDelta = np.append(
                    gammaCandidateOtherDelta,
                    (corrHatAbsMax - cj[indTmp]) / (AActiveSet - aj[indTmp])
                    - gammaCandidate,
                )

        indPlusClose = np.where(
            np.abs(compPlusInComplement - gammaCandidate) < EPS * 2
        )[0]
    ...
    def _SelectNewJoinIndex(
        ...
        assert (
            ("indHat" in joinResultDict)
            and ("indHatOther" in joinResultDict)
            and (joinResultDict["indHatOther"].size > 0)
        )
        ...
        for indHat in indHatList:
            ...
            calcPair = (
                (corrHatAbsMax - joinResultDict["cj"][indHatInt])
                / (joinResultDict["AActiveSet"] - joinResultDict["aj"][indHatInt]),
                (corrHatAbsMax + joinResultDict["cj"][indHatInt])
                / (joinResultDict["AActiveSet"] + joinResultDict["aj"][indHatInt]),
            )
            ...
            corrHatNextAbsMismatch = np.abs(
                corrHatAbsMaxNext - np.max(np.abs(corrHatNext.flat))
            )

            outputDict = {
                "indHat": str(indHatInt),
                "calcPair": str(calcPair),
                "corrHatNextAbsMismatch": str(corrHatNextAbsMismatch),
                "cj": str(joinResultDict["cj"][indHatInt]),
                "aj": str(joinResultDict["aj"][indHatInt]),
            }
    ...
    def _GetVariablesForIteration(self, y, fnConvolveWithPsfPrime):
        maxIter = self._optimSettingsDict.get(
            LarsConstants.INPUT_KEY_MAX_ITERATIONS, 500
        )
        nVerbose = self._optimSettingsDict.get(LarsConstants.INPUT_KEY_NVERBOSE, 0)
        bEnforceOneatatimeJoin = self._optimSettingsDict.get(
            LarsConstants.INPUT_KEY_ENFORCE_ONEATATIME_JOIN, False
        )

        HPrimey = fnConvolveWithPsfPrime(y)
        fnComputeCorrHat = lambda x: HPrimey - fnConvolveWithPsfPrime(
            np.reshape(x, y.shape)
        )  # 2.8
        ...
        iterObserver = self._optimSettingsDict.get(
            LarsConstants.INPUT_KEY_ITERATIONS_OBSERVER
        )
    ...
    def Iterate(self, y, fnConvolveWithPsf, fnConvolveWithPsfPrime):
        ...
        while numIter < maxIter:
            ...
            if nVerbose >= 1:
                msgBuffer.append(
                    "=== Iteration {0}: active set is {1} ===".format(
                        numIter, activeSet
                    )
                )
                msgBuffer.append(
                    "ACorr(activeSet): {0} + {1}".format(
                        corrHatAbs[activeSet[0]],
            ...
            if corrHatAbsMismatch > self.EPS * 2:
                ...
                raise RuntimeError(
                    "Iteration {0}: Abs corr hats don't match: theory is {1} whereas actual is {2} @ index/indices {3}, |delta| is {4}".format(
                        numIter,
                        corrHatAbsMax,
                        corrHatAbsMaxActual,
            ...
            activeSetActual = np.where(np.abs(corrHatAbs - corrHatAbsMax) < self.EPS)[
                0
            ]  # (2.9)
            ...
            if nVerbose >= 1:
                if activeSetActual.size > 0:
                    msgBuffer.append(
                        "ACorr(activeSetActual): {0} + {1}".format(
                            corrHatAbs[activeSetActual[0]],
                            corrHatAbs[activeSetActual]
                            - corrHatAbs[activeSetActual[0]],
                        )
                    )
                else:
            ...
            if np.setxor1d(activeSet, activeSetActual).size != 0:
                ...
                warnings.warn(
                    ...
                    + "ActiveSet is {0} vs. ActiveSetActual is {1}".format(
                        activeSet, activeSetActual
                    ),
                    RuntimeWarning,
                )
            ...
            if (XActiveSet.shape[0] != y.size) or (
                XActiveSet.shape[1] != activeSet.size
            ):
                raise RuntimeError(
                    "Expect XActiveSet.shape={0}x{1} to equal {2}x{3}".format(
                        XActiveSet.shape[0], XActiveSet.shape[1], y.size, activeSet.size
                    )
                )
            ...
            joinResult = LarsReconstructor._CalculateNewJoinIndex(
                corrHatAbsMax,
                corrHat.flat,
                activeSetResult["A"],
                aFlat,
            ...
            if joinResult["gammaHatOtherDelta"].size > 0:
                msgOutput = "gammaHat is {0} while the delta is {1}\n".format(
                    joinResult["gammaHat"], joinResult["gammaHatOtherDelta"]
                ) + "indHat is {0} while the other possibilities are {1}".format(
                    joinResult["indHat"], joinResult["indHatOther"]
                )
                ...
                (
                    lastIndexToJoinActiveSet,
                    debugMsg,
                ) = LarsReconstructor._SelectNewJoinIndex(
                    joinResult,
                    activeSetResult,
                    activeSet,
                    corrHatAbsMax,
            ...
            if nVerbose >= 1:
                msgBuffer.append(
                    "gammaHat is {0}, AActiveSet is {1}, predicted corrHatAbsMax is is {2}".format(
                        joinResult["gammaHat"], activeSetResult["A"], corrHatAbsMax
                    )
                )
            ...
            dHat[activeSet] = np.transpose(
                corrHatSign[activeSet] * np.transpose(activeSetResult["w"])
            )
                ...
                iterObserver.UpdateState(
                    {
                        ...
                        LarsIterationEvaluator.STATE_KEY_FIT_ERROR: y
                        - fnConvolveWithPsf(np.reshape(betaHatActiveSet, y.shape)),
        ...
        return {
            LarsConstants.OUTPUT_KEY_ACTIVESET: activeSet,
