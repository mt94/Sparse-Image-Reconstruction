...
class HyperparameterPick(object):
    def __init__(self, iterObserver):
        ...
        assert (len(iterObserver.HistoryState) > 0) and (
            len(iterObserver.HistoryState) == len(iterObserver.HistoryEstimate)
        )
    ...
    def GetBestEstimate(self, metricDesc, metricDeltaThres=None):
        ...
        if metricDeltaThres is not None:
            assert (
                metricDeltaThres < 0
            ), "Metric delta threshold must be strictly negative"
