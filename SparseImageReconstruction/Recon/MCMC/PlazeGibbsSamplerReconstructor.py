...
class PlazeGibbsSamplerReconstructor(AbstractMcmcSampler, AbstractReconstructor):
    ...
    def __init__(self, optimSettingsDict):
        ...
        if not (McmcConstants.INPUT_KEY_HYPERPARAMETER_PRIOR_DICT in optimSettingsDict):
            raise KeyError("Missing keys in optimSettingsDict")
        self.hyperparameterPriorDict = optimSettingsDict[
            McmcConstants.INPUT_KEY_HYPERPARAMETER_PRIOR_DICT
        ]

        # Use defaults of 2000 iterations and 300 burn-in iterations
        self.Iterations = optimSettingsDict.get(
            McmcConstants.INPUT_KEY_NUM_ITERATIONS, 2000
        )
        self.BurninSamples = optimSettingsDict.get(
            McmcConstants.INPUT_KEY_NUM_BURNIN_SAMPLES, 300
        )
        self.ThinningPeriod = optimSettingsDict.get(
            McmcConstants.INPUT_KEY_NUM_THINNING_PERIOD, 1
        )  # By default, don't do any thinning

        self.iterObserver = optimSettingsDict.get(
            McmcConstants.INPUT_KEY_ITERATIONS_OBSERVER
        )
        ...
        logging.basicConfig(
            filename="PlazeGibbsSamplerReconstructor-{0}-{1}-{2}.log".format(
    ...
    def DoSamplingSpecificXConditionedAll(
        self, w, a, ind, fitErrExcludingInd, varLast, bLogDebug=False
    ):
        assert (a > 0) and (w >= 0) and (varLast >= 0), (
            __file__ + ": DoSamplingSpecificXConditionedAll: invalid inputs"
        )
        ...
        y = (
            -muInd
            / PlazeGibbsSamplerReconstructor.CONST_SQRT_2
            / math.sqrt(etaIndSquared)
        )

        uInd = (
            (w / a)
            * mpmath.sqrt(etaIndSquared)
            * PlazeGibbsSamplerReconstructor.CONST_SQRT_HALF_PI
            * mpmath.erfc(y)
            * mpmath.exp(y * y)
        )
        ...
        assert uIndFloat >= 0, (
            ...
            + str(y)
            + " muIndComp=("
            + str(muIndComponents[0])
            + ","
            + str(muIndComponents[1])
            + ")"
            + " dotProduct="
            + str(dotProduct)
            + " hNormSquared="
            + str(self._hNormSquared[ind])
        )
        ...
        if pymc.rbernoulli(wInd):
            ...
            try:
                ...
                logging.error(msg)
                xSample = 0
                # XXX: Due to a numerical problem
            else:
                ...
                if xSample < 0:
                    ...
                    xSample = 0
                    # XXX: Also due to a numerical problem, but no exception raised
        else:
    ...
    def DoSamplingXConditionedAll(
        self, y, w, a, varLast, xLast, hxLast, bLogDebug=False
    ):
        M = xLast.size
        xNext = np.copy(xLast)
        ...
        if discrepencyNorm > discrepencyNormTol:
            raise RuntimeError(
                "|dis| is {0} > tol={1}: |hxLast| = {2}, |hxLast2| = {3}".format(
                    discrepencyNorm, discrepencyNormTol, hxLastNorm, hxLast2Norm
                )
            )
        # /// END SANITY
        ...
        for ind in range(M):
            ...
            xIndNew = self.DoSamplingSpecificXConditionedAll(
                w, a, ind, fitErrExcludingInd, varLast, bLogDebug
            )
            hxNext[:, 0] = hxNext[:, 0] + hInd * (xIndNew - xNext[ind])
            xNext[ind] = xIndNew
            # Replace the ind-th component with the newly sampled xInd
    ...
    def DoSamplingIteration(self, y):
        ...
        assert (wSample >= 0) and (wSample <= 1), (
            __file__ + ": wSample is out of bounds"
        )
        logging.info(
            "  Samp. Iter. {0}, generating wSample ~ Beta({1},{2}) ... {3:.5f}".format(
                self._samplerIter, 1 + n1, 1 + n0, wSample
            )
        )
        ...
        assert (igShapeForA > 0) and (
            igScaleForA > 0
        ), "IG shape and scale aren't strictly positive"
        ...
        for tryInd in range(5, 0, -1):
            try:
                ...
                logging.error(
                    "Couldn't generate aSample ~ IG({0},{1}) using n0={2}, n1={3}: {4}".format(
                        igShapeForA, igScaleForA, n0, n1, e.message
                    )
                )
        ...
        logging.info(
            "  Samp. Iter. {0}, generating aSample ~ IG({1:.4f},{2:.4f}) ... {3:.4e}".format(
                self._samplerIter, igShapeForA, igScaleForA, aSample
            )
        )
        ...
        xNext, hxNext = self.DoSamplingXConditionedAll(
            y, wSample, aSample, varLast, xLast, self.hx, self.nVerbose > 0
        )
        ...
        logging.info(
            "  Samp. Iter. {0}, generating varianceSample ~ IG({1:.4f},{2:.4f}) ... {3:.4e}".format(
                self._samplerIter,
                igShapeForVariance,
                igScaleForVariance,
                varianceSample,
        ...
        assert len(self.xSeq) == len(self.varianceSeq), (
            __file__ + ": DoSamplingIteration: seq length mismatch #1"
        )
        assert len(self.xSeq) == (len(self.hyperparameterSeq) + 1), (
            __file__ + ": DoSamplingIteration: seq length mismatch #2"
        )
    ...
    def DoSamplingXPrior(self, w, a, M=None):
        ...
        assert (w >= 0) and (w <= 1), (
            __file__ + ": DoSamplingXPrior: w is out of bounds"
        )
        bIsZero = pymc.rbernoulli(w, (xLen, 1))
        FLaplaceDistInv = lambda x: (
            -a * np.sign(x - 0.5) * np.log(1 - 2 * np.abs(x - 0.5))
        )
    ...
    def DoSamplingPriorVariancePrior(varMin, varMax):
        ...
        assert (varMin > 0) and (varMin < varMax), (
            __file__ + ": DoSamplingPriorVariancePrior: var{Min,Max} are invalid"
        )
    ...
    def ComputePosteriorProb(y, convMatrixObj, theta, aPriorDict, eps):
        assert ("alpha0" in aPriorDict) and ("alpha1" in aPriorDict), (
            __file__ + ": ComputePosteriorProb: missing keys in input dict"
        )
        ...
        return (
            spspecial.beta(1 + n1, 1 + n0)
            / np.sum(err * err)
            * spspecial.gamma(n1 + aPriorDict["alpha0"])
            / (np.sum(np.abs(thetaFlat)) + aPriorDict["alpha1"])
            ** (n1 + aPriorDict["alpha0"])
        )
    ...
    def SamplerSetup(self, convMatrixObj, initializationDict):
        ...
        if not ("init_theta" in initializationDict) or not (
            "init_var" in initializationDict
        ):
            raise KeyError(
                "Initialization dictionary missing keys init_theta and/or init_var"
            )

        self.xSeq = []  # Contains x^{(t)}, t=0, ..., T
        x0 = np.array(
            initializationDict["init_theta"]
        )  # Expect x0 to be either a 2-d or 3-d array.
        ...
        assert initializationDict["init_var"] >= 0, (
            __file__ + ": SamplerSetup: missing init_var in init dict"
        )
        ...
        for ind in range(M):
            ...
            assert self._hNormSquared[ind] > 0, (
                __file__ + ": SamplerSetup: norm is not strictly positive"
            )
    ...
    def SamplerGet(self, elementDesc, maxNumSamples=float("inf")):
        ...
        if not (hasattr(self, "bSamplerRun")) or (not self.bSamplerRun):
            ...
            assert len(self.xSeq) == (self.Iterations + self.BurninSamples + 1), (
                __file__ + ": SamplerGet: unexpected xSeq length"
            )
            assert len(self.varianceSeq) == len(self.xSeq), (
                __file__ + ": SamplerGet: unexpected varianceSeq length"
            )
            assert len(self.hyperparameterSeq) == (len(self.xSeq) - 1), (
                __file__ + ": SamplerGet: unexpected hyperparameterSeq length"
            )

        # If elementDesc isn't supported, return an empty list
        samples = {
            "theta": self.xSeq[(self.BurninSamples + 1) :: self.ThinningPeriod],
            "variance": self.varianceSeq[
                (self.BurninSamples + 1) :: self.ThinningPeriod
            ],
            "hyperparameter": self.hyperparameterSeq[
                self.BurninSamples :: self.ThinningPeriod
            ],
