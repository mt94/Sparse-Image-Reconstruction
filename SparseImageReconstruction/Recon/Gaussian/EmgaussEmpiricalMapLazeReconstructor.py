...
from ...Recon.Gaussian.EmgaussEmpiricalMapReconstructor import (
    EmgaussEmpiricalMapReconstructor,
)
from ...Systems.Thresholding import (
    ThresholdingHybrid,
    ThresholdingSoft,
    ThresholdingMap1CompositeSmallWeight,
    ThresholdingMap1CompositeLargeWeight,
)
...
class LazeMap1EstimateHyperparameter(AbstractEstimateHyperparameter):
    ...
    def EstimateHyperparameter(self, args):
        ...
        if len(args) == 2:
            ...
            return (
                float(thetaTildeN.size) / float(thetaTildeL1Norm),
                float(nonzeroIndicatorL0Norm) / float(thetaTildeN.size),
            )
        else:
            raise TypeError(
                "Unexpected number of args to EstimateHyperparameter: " + str(len(args))
            )
...
class EmgaussEmpiricalMapLaze1Reconstructor(EmgaussEmpiricalMapReconstructor):
    def __init__(self, optimSettingsDict):
        super(EmgaussEmpiricalMapLaze1Reconstructor, self).__init__(
            optimSettingsDict,
        ...
        if len(args) == 1:
            ...
            if self._gSup is None:
                ...
                return (
                    aHat,
                    wHat,
                    float(self._gSup) * 2 / float(aHat) * (1 / float(wHat) - 1),
                )
        else:
            raise TypeError(
                "Unexpected number of args to EstimateHyperparameter: " + str(len(args))
...
class LazeMap2DynamicThresholding(AbstractDynamicThresholding):
    ...
    def GetDynamicThreshold(self, hyperparameter, **kwargs):
        ...
        assert r >= 0
        if r >= 1:
            return ThresholdingHybrid(
                cTmp + alpha * np.sqrt(2 * np.log(r)), cTmp, bReturnTuple=True
            )
        else:
...
class EmgaussEmpiricalMapLaze2Reconstructor(EmgaussEmpiricalMapReconstructor):
    def __init__(self, optimSettingsDict, r, gSup):
        super(EmgaussEmpiricalMapLaze2Reconstructor, self).__init__(
            optimSettingsDict,
