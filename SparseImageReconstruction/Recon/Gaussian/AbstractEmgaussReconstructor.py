...
class AbstractEmgaussReconstructor(AbstractReconstructor):
    ...
    def EstimateUsingFft(self, y, convMatrixObj, *args):
        ...
        maxIter = self._optimSettingsDict.get(
            AbstractEmgaussReconstructor.INPUT_KEY_MAX_ITERATIONS, 500
        )
        tau = self._optimSettingsDict.get(AbstractEmgaussReconstructor.INPUT_KEY_TAU, 1)

        # Get the IterationsObserver object
        assert (
            AbstractEmgaussReconstructor.INPUT_KEY_ITERATIONS_OBSERVER
            in self._optimSettingsDict
        )
        iterObserver = self._optimSettingsDict[
            AbstractEmgaussReconstructor.INPUT_KEY_ITERATIONS_OBSERVER
        ]
        assert isinstance(iterObserver, AbstractIterationsObserver)

        fnUpdateIterObserver = lambda tNp1, tN, feN: iterObserver.UpdateWithEstimates(
            tNp1, tN, feN
        )
        ...
        while numIter < maxIter:
            ...
            if iterObserver.TerminateIterations:
                self._terminationReason = (
                    "Iterations observer, terminating after "
                    + str(numIter)
                    + " iterations"
                )
                break
