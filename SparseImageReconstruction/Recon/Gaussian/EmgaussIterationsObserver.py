...
class EmgaussIterationsObserver(AbstractIterationsObserver):
    ...
    def __init__(self, inputDict=None):
        ...
        if (inputDict is not None) and (
            EmgaussIterationsObserver.INPUT_KEY_TERMINATE_COND in inputDict
        ):
            self.terminateCondition = inputDict[
                EmgaussIterationsObserver.INPUT_KEY_TERMINATE_COND
            ]
        else:
            self.terminateCondition = (
                EmgaussIterationsObserver.TERMINATE_COND_THETA_DELTA_L2
            )

        if (inputDict is not None) and (
            EmgaussIterationsObserver.INPUT_KEY_TERMINATE_TOL in inputDict
        ):
            self.terminateTolerance = inputDict[
                EmgaussIterationsObserver.INPUT_KEY_TERMINATE_TOL
            ]
        else:
    ...
    def UpdateWithEstimates(self, reconArgsNp1, reconArgsN, fitErrorN=None):
        if (
            ...
            if len(reconArgsN) == 1 and len(reconArgsN) == len(reconArgsNp1):
                ...
                thetaDiff = np.reshape(
                    reconArgsNp1[0] - reconArgsN[0], (reconArgsN[0].size,)
                )
                ...
                thetaDiff = np.reshape(
                    reconArgsNp1[0] * reconArgsNp1[1] - reconArgsN[0] * reconArgsN[1],
                    (reconArgsN[0].size,),
                )
            else:
