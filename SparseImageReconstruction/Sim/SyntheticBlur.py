...
class SyntheticBlur(AbstractBlur):
    ...
    def __init__(self, blurType, blurParametersDict):
        ...
        if self._blurType == SyntheticBlur.BLUR_GAUSSIAN_SYMMETRIC_2D:
            fwhm = self._blurParametersDict.get(
                SyntheticBlur.INPUT_KEY_FWHM, SyntheticBlur.FWHM_DEFAULT
            )
            nkHalf = self._blurParametersDict.get(
                SyntheticBlur.INPUT_KEY_NKHALF, SyntheticBlur.NKHALF_DEFAULT
            )
    ...
    def BlurImage(self, theta):
        if self._blurType == SyntheticBlur.BLUR_GAUSSIAN_SYMMETRIC_2D:
            ...
            if not np.all(self._thetaShape >= self._blurPsf.shape):
                raise ValueError(
                    "theta's shape must be at least as big as the blur's shape"
                )
