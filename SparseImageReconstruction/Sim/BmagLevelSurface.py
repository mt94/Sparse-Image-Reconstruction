...
class BmagLevelSurface(object):
    ...
    def __init__(
        ...
        if (maxPhiValue < 0) or (maxPhiValue > math.pi / 2.0):
            raise ValueError(
                "Invalid value of maxPhiValue: " + str.format("{0:.4f}", maxPhiValue)
            )
        else:
    ...
    def CreatePlot(self, maxTheta, maxPhi):
        theta, phi = np.meshgrid(
            np.linspace(0, maxTheta, self._numPointsInThetaSpace),
        ...
        c = (
            9 * m2 * (1 - np.power(sinTheta, 4)) * np.power(sinPhi, 4)
        ...
        radius = np.where(
            np.logical_and(quadSol1 > 0, quadSol2 < 0), quadSol1, 0
    ...
    def SurfaceInCartesianCoordinates(self):
        ...
        sinPhi = np.sin(phi)
        return (
            radius * sinPhi * np.cos(theta),
            radius * sinPhi * np.sin(theta),
            radius * np.cos(phi),
        )
