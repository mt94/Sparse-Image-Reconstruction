...
class MrfmBlur(AbstractBlur):
    ...
    def GetXyzMeshFor2d(xSpan, z0, numPoints):
        ...
        xyzMesh = np.mgrid[
            ...
            -xSpan : (xSpan + someEpsilon * meshDelta) : meshDelta,
            z0 : (z0 + someEpsilon) : (0.9 * someEpsilon),
        ]
        return xyzMesh
    ...
    def GetXyzMeshFor3d(xSpan, z0, zMax, numPoints):
        ...
        meshDelta = np.array((2.0 * xSpan, 2.0 * xSpan, zMax - z0)) / (
            np.array(numPoints) - 1.0
        )
        someEpsilon = 0.1
        xyzMesh = np.mgrid[
            -xSpan : (xSpan + someEpsilon * meshDelta[0]) : meshDelta[0],
    ...
    def GetG(m, Bext, xMesh, yMesh, zMesh):
        ...
        a1 = (
            -90 * mPower2 * (xMesh * xMeshPower2) * zMeshPower2 / rPower12
        ...
        a4 = (
            9 * mPower2 * xMeshPower2 * zMeshPower2 / rPower10
            + 9 * mPower2 * yMeshPower2 * zMeshPower2 / rPower10
        )
        a5 = a3**2
    ...
    def GetS(Bmag, Bres, gMesh):
        ...
        indGDefined = np.where(
            np.logical_and(np.logical_not(np.isnan(gMesh)), (gMesh != 0))
        )
    ...
    # Default values
    SMALL_M_DEFAULT = (
        4 * math.pi / 3 * BIG_M_DEFAULT * (R0_DEFAULT**3)
    )  # [emu] assume spherical tip
    ...
    def __init__(self, blurType, blurParametersDict):
        ...
        self.m = blurParametersDict.get(
            MrfmBlur.INPUT_KEY_SMALL_M, MrfmBlur.SMALL_M_DEFAULT
        )
        self.Bext = blurParametersDict.get(
            MrfmBlur.INPUT_KEY_BEXT, MrfmBlur.BEXT_DEFAULT
        )
        self.Bres = blurParametersDict.get(
            MrfmBlur.INPUT_KEY_BRES, MrfmBlur.BRES_DEFAULT
        )
        ...
        if (blurType == MrfmBlur.BLUR_2D) or (blurType == MrfmBlur.BLUR_3D):
            ...
            raise NotImplementedError(
                "MrfmBlur type " + self._blurType + " hasn't been implemented"
            )
    ...
    def GetBlurPsf(self):
        ...
        psf = MrfmBlur.GetPsfVerticalCantilever(
            self.m, self.Bext, self.Bres, self.xPk, self.X, self.Y, self.Z
        )

        if self._blurType == MrfmBlur.BLUR_2D:
            psf = psf[:, :, 0]
            # Reduce 3d to 2d array
        ...
        for dimInd in range(len(blurShift)):
            blurShift[dimInd] = min(psfSupport[dimInd]) + math.floor(
                (max(psfSupport[dimInd]) - min(psfSupport[dimInd])) / 2
            )
        if len(blurShift) == 3:
    ...
    def BlurImage(self, theta):
        ...
        if not np.all(self._thetaShape >= self._blurPsf.shape):
            raise ValueError(
                "theta's shape: "
                + str(self._thetaShape)
                + " must be at least as big as the blur's shape: "
                + str(self._blurPsf.shape)
            )

        if self._blurType == MrfmBlur.BLUR_2D:
            y = np.fft.ifft2(
                np.multiply(np.fft.fft2(self.BlurPsfInThetaFrame), np.fft.fft2(theta))
            )
        elif self._blurType == MrfmBlur.BLUR_3D:
            y = np.fft.ifftn(
                np.multiply(np.fft.fftn(self.BlurPsfInThetaFrame), np.fft.fftn(theta))
            )
        # Since _blurType was checked in __init__, there's no need to have an else to handle unimplemented
        # values.

        return y.real
        # Discard the complex part
