...
class Mrfm2dBlurWithNoise(AbstractBlurWithNoise):
    ...
    def __init__(self, optimizer, simParametersDict):
        super(Mrfm2dBlurWithNoise, self).__init__(
            optimizer,
            simParametersDict,
            "2-d Mrfm blur with additive Gaussian noise example",
        )
        self.debugMessages = []
    ...
    def GetBlur(self):
        ...
        blurEx = MrfmBlurExample(
            self._optimizer, (numPointsInMesh,), MrfmBlur.BLUR_2D, ""
        ).RunExample()
    ...
    def GetImageGenerator(self):
        ...
        igBorderWidth = (
            int(math.ceil((max(self._psfSupport[0]) - min(self._psfSupport[0])) / 2.0))
            + 1
        )
        ...
        if (self.ImageDiscreteNonzeroValues is not None) and (
            len(self.ImageDiscreteNonzeroValues) > 0
        ):
        ...
        self.debugMessages.append(
            "Border width in image generator is {0}".format(igBorderWidth)
        )
        return ig
    ...
    def RunExample(self):
        ...
        self.debugMessages.append(
            "Blur shift is: {0}".format(self.channelChain.channelBlocks[1].BlurShift)
        )

    def Plot(self):
        # In order to remove the shift, must access the SyntheticBlur block in the channel chain
        blurredImageWithNoiseForDisplay = self.channelChain.channelBlocks[
            1
        ].RemoveShiftFromBlurredImage(self.blurredImageWithNoise)

        # Random sparse image
        plt.figure(1)
        plt.imshow(self.channelChain.intermediateOutput[0], interpolation="none")
        plt.colorbar()
        # Blur
        plt.figure()
        plt.imshow(self.blurPsfInThetaFrame, interpolation="none")
        plt.colorbar()
...
if __name__ == "__main__":
    ...
    ex = Mrfm2dBlurWithNoise(
        Mrfm2dBlurWithNoise.GetBlurParameterOptimizer(),
        {
    ...
    print(
        "Channel block timing [ms]: {0}".format(
            ", ".join(
