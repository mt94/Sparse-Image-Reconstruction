...
class AbstractBlurWithNoise(AbstractExample):
    ...
    @property
    def NoiseSigma(self):
        return self._simParametersDict.get(
    ...
    def SnrDb(self):
        return self._simParametersDict.get(
            AbstractAdditiveNoiseGenerator.INPUT_KEY_SNRDB
        )

    @property
    def NumNonzero(self):
        try:
            numNonzero = self._simParametersDict[
                AbstractImageGenerator.INPUT_KEY_NUM_NONZERO
            ]
        except KeyError:
            numNonzero = 8
        return numNonzero
    ...
    def ImageType(self):
        try:
            imageType = self._simParametersDict[
                AbstractImageGenerator.INPUT_KEY_IMAGE_TYPE
            ]
        except KeyError:
    ...
    def RunExample(self):
        if (
            self._simParametersDict.get(AbstractBlurWithNoise.RUN_ONCE_ONLY, False)
            and self._bRunAlready
        ):
        ...
        # image generator
        channelChain.channelBlocks.append(blr)
        # blur
