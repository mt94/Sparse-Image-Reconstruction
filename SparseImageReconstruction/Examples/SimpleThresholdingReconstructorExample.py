...
class SimpleThresholdingReconstructorExample(AbstractReconstructorExample):
    ...
    _concreteReconstructor = {
        "landweber": (
            EmgaussFixedMstepReconstructor,
            Thresholding.ThresholdingIdentity,
        ),
        "landweber_nonneg": (
            EmgaussFixedMstepReconstructor,
            Thresholding.ThresholdingIdentityNonnegative,
        ),
    }

    def __init__(self, estimatorDesc, maxIterations=2e5):
        super(SimpleThresholdingReconstructorExample, self).__init__(
            "Simple thresholding reconstructor example"
        )
        if (
            estimatorDesc
            not in SimpleThresholdingReconstructorExample._concreteReconstructor
        ):
            raise NotImplementedError(
                estimatorDesc + " is an unrecognized reconstructor"
            )
        else:
    ...
    def RunExample(self):
        ...
        params = SimpleThresholdingReconstructorExample._concreteReconstructor[
            self.estimatorDesc
        ]
        ...
        with Timer() as t:
            self._thetaEstimated = reconstructor.Estimate(
                ...
                InitialEstimatorFactory.GetInitialEstimator("Hty").GetInitialEstimate(
                    y, psfRepH
                ),
            )[0]
...
def RunReconstructorUsingExpObj(param, experimentObj):
    ...
    exReconstructor = SimpleThresholdingReconstructorExample(
        reconstructorDesc, maxIterations
    )
