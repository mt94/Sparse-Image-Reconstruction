...
class LarsReconstructorOnExample(AbstractReconstructorExample):
    ...
    GAUSSIAN_BLUR_WITH_NOISE_DUMP_FILE = ""
    #'c:\\tmp\\LeastAngleRegressionOnExampleGbwn.dump'
        ...
        if not self.bRestoreSim:
            ...
            if LarsReconstructorOnExample.GAUSSIAN_BLUR_WITH_NOISE_DUMP_FILE:
                pickle.dump(
                    self.experimentObj,
                    open(
                        LarsReconstructorOnExample.GAUSSIAN_BLUR_WITH_NOISE_DUMP_FILE,
                        "wb",
                    ),
                )
        else:
            ...
            if LarsReconstructorOnExample.GAUSSIAN_BLUR_WITH_NOISE_DUMP_FILE:
                self.experimentObj = pickle.load(
                    open(
                        LarsReconstructorOnExample.GAUSSIAN_BLUR_WITH_NOISE_DUMP_FILE,
                        "rb",
                    )
                ...
                raise NameError(
                    "Cannot restore object when dump filename isn't defined"
                )
    ...
    def RunExample(self):
        ...
        psfRepH = self.experimentObj.channelChain.channelBlocks[
            1
        ...
        self.iterObserver.ThetaTrue = (
            np.array(self.experimentObj.channelChain.intermediateOutput[0])
            * self.thetaScalingFactor
        )
        ...
        reconstructor = LarsReconstructorFactory.GetReconstructor(
            self.reconstructorDesc, optimSettingsDict
        )
    ...
    def PrintOutputSummary(self):
        activeSetDisplay = [
            "{0}".format(x)
            for x in self.reconResult[LarsConstants.OUTPUT_KEY_ACTIVESET]
        ]
        print("Active set: {0}".format(" ".join(activeSetDisplay)))

        if LarsConstants.OUTPUT_KEY_SIGN_VIOLATION_NUMITER in self.reconResult:
            print(
                "Number of Lars-Lasso iteration(s) with a sign violation: {0}".format(
    ...
    def PrintOutputIterations(self):
        ...
        for h in self.iterObserver.HistoryState:
            ...
            msg += fmtString.format(
                h[LarsIterationEvaluator.OUTPUT_METRIC_FITERR_SS],
            ...
            if (LarsIterationEvaluator.OUTPUT_METRIC_THETA_PROPEXPL in h) and (
                LarsIterationEvaluator.OUTPUT_METRIC_MU_PROPEXPL in h
            ):
                msg += ", theta/mu prop. expl.: {0:.5f}/{1:.5f}".format(
                    h[LarsIterationEvaluator.OUTPUT_METRIC_THETA_PROPEXPL],
                    h[LarsIterationEvaluator.OUTPUT_METRIC_MU_PROPEXPL],
                )
            if LarsIterationEvaluator.OUTPUT_CRITERION_L1_SURE in h:
                lassoSureCriterionCurr = h[
                    LarsIterationEvaluator.OUTPUT_CRITERION_L1_SURE
                ]
                msg += ", SURE criterion: {0:.5f}".format(lassoSureCriterionCurr)
                if lassoSureCriterionPrev is not None:
                    msg += " ({0:.3f})".format(
                        lassoSureCriterionCurr - lassoSureCriterionPrev
                    )
                lassoSureCriterionPrev = lassoSureCriterionCurr
                # Update
    ...
    def PlotMetricVsStages(self, fignumStart):
        ...
        blurredImageWithNoiseForDisplay = experimentObj.channelChain.channelBlocks[
            1
        ].RemoveShiftFromBlurredImage(experimentObj.blurredImageWithNoise)

        blurredImageWithNoiseForDisplayZeroMean = (
            blurredImageWithNoiseForDisplay
            - np.mean(blurredImageWithNoiseForDisplay.flat)
            * np.ones(blurredImageWithNoiseForDisplay.shape)
        )

        assert np.mean(blurredImageWithNoiseForDisplayZeroMean) < 1.0e-11

        estimatedMu = np.reshape(
            self.reconResult[LarsConstants.OUTPUT_KEY_MUHAT_ACTIVESET],
            experimentObj.blurredImageWithNoise.shape,
        )

        estimatedMuForDisplay = experimentObj.channelChain.channelBlocks[
            1
...
def PlotTheta2d(fignumStart, iterObserver, indBest, thetaBest, thetaScalingFactor=1):
    ...
    plt.imshow(
        np.reshape(thetaBest / float(thetaScalingFactor), iterObserver.ThetaTrue.shape),
        interpolation="none",
    )
    plt.colorbar()
...
def RunReconstructor(param, imageDiscreteNzvalues=None, bPlot=False):
    ...
    exReconstructor = LarsReconstructorOnExample(
        reconstructorDesc, iterObserver, maxIterations, bRestoreSim=False
    )
    ...
    if (imageDiscreteNzvalues is not None) and (len(imageDiscreteNzvalues) > 0):
        blurWithNoiseParams[
            AbstractImageGenerator.INPUT_KEY_IMAGE_DISCRETE_NZVALUES
        ] = imageDiscreteNzvalues

    exReconstructor.experimentObj = BlurWithNoiseFactory.GetBlurWithNoise(
        experimentDesc, blurWithNoiseParams
    )
    ...
    indBest, thetaBest = hparamPick.GetBestEstimate(
        LarsIterationEvaluator.OUTPUT_CRITERION_L1_SURE, -0.1
    )
    ...
    if bPlot and (len(iterObserver.ThetaTrue.shape) == 2):
        PlotTheta2d(
            2, iterObserver, indBest, thetaBest, exReconstructor.thetaScalingFactor
        )
        plt.show()

    perfCriteria = ReconstructorPerformanceCriteria(
        exReconstructor.Theta, np.reshape(thetaBest, exReconstructor.Theta.shape)
    )
