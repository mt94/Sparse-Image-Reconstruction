...
from ..Recon.Gaussian.EmgaussEmpiricalMapLazeReconstructor import (
    EmgaussEmpiricalMapLaze1Reconstructor,
    EmgaussEmpiricalMapLaze2Reconstructor,
)
...
class EmgaussEmpiricalMapLazeReconstructorOnExample(AbstractReconstructorExample):
    ...
    def __init__(self, estimatorDesc, noiseSigma=None, snrDb=None, r=None, gSup=None):
        super(EmgaussEmpiricalMapLazeReconstructorOnExample, self).__init__(
            "Empirical MAP LAZE Reconstructor example"
        )

        if (
            estimatorDesc
            not in EmgaussEmpiricalMapLazeReconstructorOnExample._concreteMapReconstructor
        ):
            raise NotImplementedError(
                estimatorDesc + " is an unrecognized MAP reconstructor"
            )
        else:
    ...
    def RunExample(self):
        ...
        optimSettingsDict = {
            ...
            AbstractEmgaussReconstructor.INPUT_KEY_ALPHA: self.noiseSigma
            / math.sqrt(psfSpectralRadius),
        ...
        clsReconstructor = (
            EmgaussEmpiricalMapLazeReconstructorOnExample._concreteMapReconstructor[
                self.estimatorDesc
            ]
        )

        initialEstimate = InitialEstimatorFactory.GetInitialEstimator(
            "Hty"
        ...
        with Timer() as t:
            self._thetaEstimated = reconstructor.Estimate(
                y, ConvolutionMatrixUsingPsf(psfRepH), *estimateArg
            )
...
def RunReconstructor(param, imageDiscreteNzvalues=None):
    ...
    if snrDb is not None:
        ...
        experimentObj = BlurWithNoiseFactory.GetBlurWithNoise(
...
def RunReconstructorUsingExpObj(param, experimentObj):
    ...
    if len(param) == 5:
        ...
        exReconstructor = EmgaussEmpiricalMapLazeReconstructorOnExample(
            mapRtorDesc, snrDb=snrDb
        )
    elif len(param) == 6:
        mapRtorDesc = "map2"
        [_experimentDesc, _imageType, _imageShape, snrDb, _numNonzero, gSup] = param
        exReconstructor = EmgaussEmpiricalMapLazeReconstructorOnExample(
            mapRtorDesc, snrDb=snrDb, r=0, gSup=gSup
        )
    else:
        raise NotImplementedError()
    ...
    perfCriteria = ReconstructorPerformanceCriteria(
        exReconstructor.Theta, exReconstructor.ThetaEstimated
    )
    ...
    if not bRunPool:
        ...
        print(
            fmtString.format(
                ...
                singleResult["normalized_l2_error_norm"],
                singleResult["normalized_detection_error"],
                singleResult["normalized_l0_norm"],
