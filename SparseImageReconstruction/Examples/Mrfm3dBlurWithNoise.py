...
class Mrfm3dBlurWithNoise(AbstractBlurWithNoise):
    ...
    def __init__(self, optimizer, simParametersDict):
        super(Mrfm3dBlurWithNoise, self).__init__(
            optimizer,
            simParametersDict,
            "3-d Mrfm blur with additive Gaussian noise example",
        )
        assert len(self.ImageShape) == 3
        assert self.ImageShape[0] == self.ImageShape[1]
        # Constrain the x and y dims to have equal length
        self.debugMessages = []
    ...
    def GetBlur(self):
        blurEx = MrfmBlurExample(
            self._optimizer,
            (self.ImageShape[0], self.ImageShape[2]),
            MrfmBlur.BLUR_3D,
            "",
        ).RunExample()
    ...
    def GetImageGenerator(self):
        ...
        igBorderWidth = [
            (int(math.ceil((max(suppVec) - min(suppVec)) / 2.0)) + 1)
            for suppVec in self._psfSupport[0:2]
        ]
    ...
    def Plot(self, plotType, fignumStart=1):
        ...
        imgToPlot = {
            ...
            Mrfm3dBlurWithNoise.IMG_BLURRED: self.channelChain.channelBlocks[
                1
...
if __name__ == "__main__":
    ...
    ex = Mrfm3dBlurWithNoise(
        Mrfm3dBlurWithNoise.GetBlurParameterOptimizer(),
        {
