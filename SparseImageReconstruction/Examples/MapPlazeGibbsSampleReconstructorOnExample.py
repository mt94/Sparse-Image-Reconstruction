...
from ..Recon.MCMC.MapPlazeGibbsSamplerReconstructor import (
    MapPlazeGibbsSamplerReconstructor,
)
...
class MapPlazeGibbsSampleReconstructorOnExample(AbstractReconstructorExample):
    ...
    def __init__(self, iterObserver, optimSettingsDict, snrDb=None):
        super(MapPlazeGibbsSampleReconstructorOnExample, self).__init__(
            "MAP P-LAZE Gibbs Sampler"
        )
    ...
    def RunExample(self):
        ...
        print(
            "SIM: SNR is {0} dB => noise var is {1}".format(
                self.snrDb, (self.experimentObj.NoiseSigma) ** 2
            )
        )
        ...
        optimSettingsDict = {
            McmcConstants.INPUT_KEY_EPS: ComputeEnvironment.EPS,
            McmcConstants.INPUT_KEY_HYPERPARAMETER_PRIOR_DICT: {
                "alpha0": hyperparamPriorAlpha,
                "alpha1": hyperparamPriorAlpha,
            },
            McmcConstants.INPUT_KEY_ITERATIONS_OBSERVER: self.iterObserver,
            McmcConstants.INPUT_KEY_NUM_ITERATIONS: self.optimSettingsDict.get(
                McmcConstants.INPUT_KEY_NUM_ITERATIONS, 1000
            ),
            McmcConstants.INPUT_KEY_NUM_BURNIN_SAMPLES: self.optimSettingsDict.get(
                McmcConstants.INPUT_KEY_NUM_BURNIN_SAMPLES, 300
            ),
            McmcConstants.INPUT_KEY_NUM_THINNING_PERIOD: self.optimSettingsDict.get(
                McmcConstants.INPUT_KEY_NUM_THINNING_PERIOD, 1
            ),
            McmcConstants.INPUT_KEY_NVERBOSE: 0,
        }
        reconstructor = MapPlazeGibbsSamplerReconstructor(
            ComputeEnvironment.EPS, optimSettingsDict
        )

        # NOTE: assume theta.shape = y.shape, so M = P
        M = y.size

        initTheta = np.reshape(
            reconstructor.DoSamplingXPrior(self.wInit, self.aInit, M), y.shape
        )
        initVar = reconstructor.DoSamplingPriorVariancePrior(
            self.varInitMin, self.varInitMax
        )
        #         initVar = (self.experimentObj.NoiseSigma) ** 2
        #         initVar = 1e-2

        print(
            "INITIAL CONDS.: hyper: w={0}, a={1}; var: {2}".format(
                self.wInit, self.aInit, initVar
            )
        )
        ...
        with Timer() as t:
            self._thetaEstimated = reconstructor.Estimate(
                y, convMatrixObj, initializationDict
            )
    ...
    def ReselectEstimate(self, maxNumSamples):
        ...
        self._thetaEstimated = self._reconstructor.SelectOptimum(
            self.NoisyObs, self.BlurOperator, maxNumSamples
        )
    ...
    def Plot2d(self, fignumStart):
        ...
        plt.figure()
        plt.imshow(
            np.reshape(self.ThetaEstimated, self.Theta.shape), interpolation="none"
        )
        plt.colorbar()
...
def RunReconstructor(param, imageDiscreteNzvalues=None, bPlot=False):
    ...
    iterEvaluator = McmcIterationEvaluator(
        ComputeEnvironment.EPS,
