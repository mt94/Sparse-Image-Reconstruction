...
class MrfmBlurExample(AbstractExample):
    ...
    def Plot(self, fignum):
        ...
        if self._blurType == MrfmBlur.BLUR_2D:
            ...
            zDelta = (self._zRange[1] - self._zRange[0]) / (
                self._numPointsInMesh[1] - 1
            )
            # 3-d blur
            ...
            for zInd in range(blurPsfShape[2]):
                ...
                plt.imshow(
                    xySlice, interpolation="none", extent=(-xSpan, xSpan, -xSpan, xSpan)
                )
    ...
    def RunExample(self):
        ...
        if self._blurType == MrfmBlur.BLUR_2D:
            ...
            xyzMesh = MrfmBlur.GetXyzMeshFor2d(
                self._opti.xSpan, self._opti.z0, self._numPointsInMesh[0]
            )
        elif self._blurType == MrfmBlur.BLUR_3D:
            assert len(self._numPointsInMesh) == 2
            # The #pts in the x and y dimensions are the same
            bmLevelSurface = BmagLevelSurface(
                self._opti.Bext, self._opti.Bres**2.0, self._opti.m
            )
            zMaxInMesh = bmLevelSurface.GetSupport["zSupport"][1] * 1.1
            self._zRange = (self._opti.z0, zMaxInMesh)
            xyzMesh = MrfmBlur.GetXyzMeshFor3d(
                self._opti.xSpan,
                self._opti.z0,
                zMaxInMesh,
                (
