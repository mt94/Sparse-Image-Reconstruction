...
class Gaussian2dBlurWithNoise(AbstractBlurWithNoise):
    ...
    def __init__(self, simParametersDict):
        super(Gaussian2dBlurWithNoise, self).__init__(
            None,
            simParametersDict,
            "Gaussian SyntheticBlur with additive Gaussian noise example",
        )
    ...
    def GetBlur(self):
        ...
        return SyntheticBlur(
            SyntheticBlur.BLUR_GAUSSIAN_SYMMETRIC_2D, blurParametersDict
        )
...
if __name__ == "__main__":
    ...
    print(
        "Spectral radius of H*H^T is:", gbNormalizer.GetSpectralRadiusGramMatrixRowsH()
    )

    # In order to remove the shift, must access the SyntheticBlur block in the channel chain
    blurredImageWithNoiseForDisplay = ex.channelChain.channelBlocks[
        1
